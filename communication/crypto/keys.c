...
// This is not thread-safe! You'll need to use a mutex if you want this to
// work in a multi-threaded world.
// But its worth noting that duplicating the group instead of allocating
// it on each pass reduced the execution time by 50%!
...
EC_KEY * ecies_key_create_public_octets(unsigned char *octets,
                                        size_t length)
{
    EC_GROUP *group;
    EC_KEY *key = NULL;
