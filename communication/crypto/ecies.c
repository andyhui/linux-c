...
void * ecies_key_derivation(const void *input,
                            size_t ilen,
                            void *output,
                            size_t *olen)
{
...
secure_t * ecies_encrypt(char *key, unsigned char *data, size_t length)
{
    ...
    // Make sure there is enough key material for the symmetric ciphers.
    if ((key_length = EVP_CIPHER_key_length(ECIES_CIPHER)) * 2 >
            SHA512_DIGEST_LENGTH)
    {
        printf("The key derivation method will not produce enough envelope \
                key material for the chosen ciphers. \
                {envelope = %i / required = %zu}",
    ...
    {
        printf("An error occurred while trying to generate ephemeral key.\n");
        EC_KEY_free(user);
        return NULL;
    }

    /* Use the intersection of the provided keys to generate the envelope data
     * used by the ciphers below.
     * The ecies_key_derivation() function uses SHA 512 to ensure we have a 
     * sufficient amount of envelope key material and that the material 
     * created is sufficiently secure.
     */
    else if (ECDH_compute_key(envelope_key,
             SHA512_DIGEST_LENGTH,
             EC_KEY_get0_public_key(user),
             ephemeral,
    ...
    // Store the public key portion of the ephemeral key.
                ...
                secure_key_data(cryptex),
                envelope_length,
                NULL) != envelope_length)
    {
        printf("An error occurred while trying to record the public portion \
                of the envelope key. {error = %s}\n",
                ERR_error_string(ERR_get_error(), NULL));
        EC_KEY_free(ephemeral);
        EC_KEY_free(user);
        secure_free(cryptex);
    ...
    // The envelope key has been stored so we no longer need to keep
    // the keys around.
    EC_KEY_free(ephemeral);
    EC_KEY_free(user);
    ...
    {
        printf("An error occurred while trying to secure the data using the \
                chosen symmetric cipher. {error = %s}\n",
                ERR_error_string(ERR_get_error(), NULL));
    ...
    // we either have a partial block remaining, or an error occurred.
        ...
        // Make sure all that remains is a partial block,
        // and their wasn't an error.
        ...
        // Copy the remaining data into our partial block buffer.
        // The memset() call ensures any extra bytes will be zero'ed out.
        memset(block, 0, EVP_MAX_BLOCK_LENGTH);
        memcpy(block, data + body_length, length - body_length);

        // Advance the body pointer to the location of the remaining space,
        // and calculate just how much room is still available.
        body += body_length;
        ...
        // Pass the final partially filled data block into the cipher as
        // a complete block. The padding will be removed during
        // the decryption process.
        else if (EVP_EncryptUpdate(&cipher, body,
                    &body_length, block, block_length) != 1)
        {
            printf("Unable to secure the data using the chosen symmetric \
                    cipher. {error = %s}\n",
                    ERR_error_string(ERR_get_error(), NULL));
    ...
    // Advance the pointer, then use pointer arithmetic to calculate how much
    // of the body buffer has been used.
    // The complex logic is needed so that we get the correct status
    // regardless of whether there was a partial data block.
    body += body_length;
    if ((body_length = secure_body_length(cryptex) -
                (body - secure_body_data(cryptex))) < 0)
    {
        printf("The symmetric cipher overflowed!\n");
        EVP_CIPHER_CTX_cleanup(&cipher);
        secure_free(cryptex);
    ...
    // Generate an authenticated hash which can be used to validate the data
    // during decryption.
    HMAC_CTX_init(&hmac);
    mac_length = secure_mac_length(cryptex);

    // At the moment we are generating the hash using encrypted data.
    // At some point we may want to validate the original text instead.
    if (HMAC_Init_ex(&hmac, envelope_key + key_length,
                key_length, ECIES_HASHER, NULL) != 1 ||
            HMAC_Update(&hmac, secure_body_data(cryptex),
...
unsigned char * ecies_decrypt(char *key, secure_t *cryptex, size_t *length)
{
    ...
    // Create the ephemeral key used specifically for this block of data.
    else if (!(ephemeral = ecies_key_create_public_octets(
                    secure_key_data(cryptex), secure_key_length(cryptex))))
    {
        printf("An error occurred while trying to recreate \
                the ephemeral key.\n");
        EC_KEY_free(user);
        return NULL;
    }
    ...
    if (mac_length != secure_mac_length(cryptex) ||
            memcmp(md, secure_mac_data(cryptex), mac_length))
    {
        printf("The authentication code was invalid! The ciphered data has \
                been corrupted!\n");
        return NULL;
    }
    ...
    {
        printf("An error occurred while trying to allocate memory for the \
                decrypted data.\n");
        return NULL;
    }
    ...
    if (EVP_DecryptInit_ex(&cipher, ECIES_CIPHER, NULL, envelope_key, iv) != 1
            ...
            || EVP_DecryptUpdate(&cipher, block, &output_length,
                secure_body_data(cryptex), secure_body_length(cryptex)) != 1)
    {
    ...
    {
        printf("The symmetric cipher failed to properly decrypt the correct \
                amount of data!\n");
        EVP_CIPHER_CTX_cleanup(&cipher);
        free(output);
        return NULL;
